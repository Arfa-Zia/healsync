...
//  HealSync
//
//  Created by Arfa on 14/01/2026.
//

import UIKit

class ClientWelcomeVC: UIViewController{

    private var titleLabel = TitleLabel(text: "Welcome" , fontSize: 44)
    private var subtitleLabel = SubtitleLabel(text: "Your journey to healing starts here" , noOfLines: 2 , fontSize: 18)
    private var gradientLayer = CAGradientLayer()
    
    private var backgroundImg: UIImageView = {
        let img = UIImageView()
        img.translatesAutoresizingMaskIntoConstraints = false
        img.image = UIImage(named: "client-welcome-img")
    ...
    private func transistToNextScreen(){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3){
            let nextVC = ClientAddProfileDetailsVC()
