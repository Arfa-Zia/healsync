...
class ClientEditProfileVC: UIViewController {
    ...
    // MARK: - Fields
    private let contactField  = ClientEditProfileVC.makeField(placeholder: "Contact Number",
                                                               icon: "phone.fill",
                                                               keyboardType: .phonePad)
    private let dobField      = ClientEditProfileVC.makeField(placeholder: "Date of Birth",
                                                               icon: "calendar")
    private let genderField   = ClientEditProfileVC.makeField(placeholder: "Gender",
                                                               icon: "person.fill")
    private let locationField = ClientEditProfileVC.makeField(placeholder: "Location",
                                                               icon: "mappin.circle.fill")
    private let guardianNameField    = ClientEditProfileVC.makeField(placeholder: "Guardian Name",
                                                                      icon: "person.2.fill")
    private let guardianContactField = ClientEditProfileVC.makeField(placeholder: "Guardian Contact",
    ...
    private func fetchUserData() {
        ...
        db.collection("users").document(uid).getDocument { [weak self] snap, error in
            ...
            DispatchQueue.main.async {
                ...
                guard let snap = snap, snap.exists, let client = ClientModel(document: snap) else {
                    print("Error: User document does not exist")
                    return
                }
                self.client = client

                self.contactField.text         = client.contactNumber
                self.dobField.text             = client.dob
                self.genderField.text          = client.gender
                self.locationField.text        = client.location
                self.guardianNameField.text    = client.guardianName
                self.guardianContactField.text = client.guardianContact
                self.avatarInitialLabel.text   = String(client.fullName.prefix(1)).uppercased()

                // Pre-select gender in picker
                if let gender = client.gender,
                   let idx = self.genders.firstIndex(of: gender) {
                    self.genderPicker.selectRow(idx, inComponent: 0, animated: false)
                }

                // Pre-select location in picker
                if let location = client.location,
                ...
                if let dobStr = client.dob, !dobStr.isEmpty {
    ...
    @objc private func handleSave() {
        ...
        func commitData(imageURL: String? = nil) {
            var updates: [String: Any] = [
                ...
                "guardianContact": guardianContactField.text ?? ""
            ]
...
extension ClientEditProfileVC: UIPickerViewDataSource, UIPickerViewDelegate {
...
extension ClientEditProfileVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
