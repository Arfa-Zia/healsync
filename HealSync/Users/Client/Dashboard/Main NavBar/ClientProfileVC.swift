...
class ClientProfileVC: UIViewController {
    ...
    private let menuItems: [(String, String, UIColor)] = [
        ("clock.arrow.circlepath", "Session History",   UIColor(hex: "#4FC3D8")),
        ("slider.horizontal.3",   "Preferences",        UIColor(hex: "#5BB8A0")),
        ("gearshape.fill",        "Account Settings",   UIColor(hex: "#7B9FD4")),
        ("questionmark.circle.fill", "Help & Support",  UIColor(hex: "#F0A070"))
    ]
    ...
    @objc private func handleEditProfile() {
        navigationController?.pushViewController(ClientEditProfileVC(), animated: true)
    }
    ...
    @objc private func menuItemTapped(_ sender: UITapGestureRecognizer) {
        ...
        switch index {
        case 0:
            let vc = SessionHistoryVC(role: .patient)
            navigationController?.pushViewController(vc, animated: true)
        case 1:
            navigationController?.pushViewController(ClientPreferencesVC(), animated: true)
        case 2:
